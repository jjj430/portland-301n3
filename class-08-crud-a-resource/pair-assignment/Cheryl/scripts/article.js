(function(module) {
  ...
  Article.createTable = function(callback) {
    webDB.execute(
      ...
      + 'body VARCHAR(250), '
      + 'publishedOn DATETIME);', // what SQL command do we run here inside these quotes?
      function(result) {
      ...
      [
        {
          'sql': 'INSERT INTO articles '
             + '(title, category, author, authorUrl, body, publishedOn) '
             + "VALUES "
             + "('?', '?', '?', '?', '?', '?');",
          'data': [this.title, this.category, this.author, this.authorUrl, this.body, this.publishedOn]
        }
      ],
      callback
    );
  ...
  Article.prototype.deleteRecord = function(callback) {
    webDB.execute(
      [
        {
          'sql': 'DELETE FROM articles WHERE id=?;', // <----finish the command here, inside the quotes.
          'data': [this.id]
        }
      ],
      callback
  ...
  Article.prototype.updateRecord = function(callback) {
    webDB.execute(
      [
        {
          'sql': "UPDATE articles "
          + "SET title='?', category='?', author='?', authorUrl='?', body='?', publishedOn='?' "
          + "WHERE id=?;", // <----finish the command here, inside the quotes.
          'data': [this.title, this.category, this.author, this.authorUrl, this.body, this.publishedOn, this.id]
        }
      ],
      callback
    );
