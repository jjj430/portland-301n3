(function(module) {
  ...
  // J+D: Insert an article instance into the database:
  ...
  // J+D: Update an article instance, overwriting it's properties into the corresponding record in the database:
  ...
  Article.fetchAll = function(callback) {
    // J+D: fill these quotes to 'select' our table.
      ...
      if (rows.length) {
        ...
        $.getJSON('data/hackerIpsum.json', function(rawData) {
          ...
          rawData.forEach(function(item) {
            ...
            // J+D: Cache the newly-instantiated article in the DB: (what can we call on each 'article'?)
            article.insertRecord();

          });
          // Now get ALL the records out the DB, with their database IDs:
          webDB.execute('SELECT * FROM articles', function(rows) { // J+D: select our now full table
            // J+D: Now, 1st - instanitate those rows with the .loadAll function,
            // and 2nd - pass control to the view by calling whichever function argument was passed in to fetchAll.
            Article.loadAll(rows);
            callback();
          });
