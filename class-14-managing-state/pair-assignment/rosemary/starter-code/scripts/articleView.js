(function(module) {
  ...
  //compiling handlebars template, get each author and make an option for each, and appends it to select
  ...
  articleView.populateFilters = function() {
    ...
    if ($('#author-filter option').length < 2) { // Prevent duplication-if not populated, then populate it
  ...
  // COMMENT: What does this method do?  What is it's execution path?  
  //one- only done once, event listener-if select changes then run this function, filter says author or cat filter, 
  //tells which page to go to
  // dynamically creating the route
  ...
  // COMMENT: What does this method do?  What is it's execution path?
  //shows article and hides siblings
  articleView.index = function(articles) {  // article init function
    $('#articles').show().siblings().hide();

    $('#articles article').remove();  //make sure empty
    articles.forEach(function(a) {    // turns it into DOM
      $('#articles').append(render(a));
    });

    articleView.populateFilters();   //populates the filter
    // COMMENT: What does this method do?  What is it's execution path?
    // handles which filter is selected - author or category
